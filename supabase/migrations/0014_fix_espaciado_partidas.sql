-- Fix: el check "Partidas demasiado seguidas para ser reales" re-escaneaba TODO
-- el historial de leaderboard_events en cada submit, no solo lo nuevo. Una vez
-- que un perfil disparaba el check una vez (ej. jugando rápido durante pruebas),
-- ese mismo par de eventos viejos quedaba en la tabla para siempre y volvía a
-- disparar la excepción en cada intento futuro — bloqueo permanente, sin forma
-- de recuperarse.
--
-- Además el umbral (mitad de segundos_min, 30-45s) era muy agresivo: repetir un
-- minijuego para superar tu propio récord en menos de eso es normal, no trampa.
--
-- Cambios:
--   1. El check solo considera pares donde al menos un evento es NUEVO en este
--      submit (v_nuevos_ts) — un par viejo ya aceptado no vuelve a bloquear.
--   2. Umbral fijo de 5 segundos en vez de segundos_min * 0.5 — sigue
--      atrapando envíos instantáneos/con script, pero no partidas humanas rápidas.
...
begin
  ...
  -- Espaciado temporal de los juegos: dos partidas no pueden solaparse. Solo se
  -- evalúan pares donde al menos un evento es nuevo en este submit, así un par
  -- ya aceptado en el pasado no vuelve a bloquear submits futuros.
  select max(t.ocurrido_en) into v_ultimo_juego
  from (
    select ocurrido_en,
  ...
  where t.anterior is not null
    and t.ocurrido_en - t.anterior < interval '5 seconds'
    and (t.ocurrido_en = any(v_nuevos_ts) or t.anterior = any(v_nuevos_ts));
