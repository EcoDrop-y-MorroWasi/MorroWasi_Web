-- Rebalancea la curva de PEW por etapa del Wasi. Antes la etapa 10 pedía
-- 1,000,000 PEW (WASI_STAGE_THRESHOLDS en src/data/mock.ts) — al ritmo real de
-- misiones diarias/semanales (~42 EXP/día), eso tomaba siglos. Nueva curva:
-- [0, 1000, 2000, 4000, 6000, 8000, 12000, 14000, 18000, 22000].
--
-- submit_leaderboard_score() calcula la columna `etapa` con su propia copia de
-- esta curva (ver comentario "Misma curva que calcWasiStage()" en 0009) — sin
-- este espejo, el ranking mostraría una etapa distinta a la que ve cada quien
-- en su propio Wasi. Se recrea la función entera porque plpgsql no permite
-- reemplazar solo el cuerpo de un case.
...
begin
  ...
  set exp = exp_total,
      ...
      -- Misma curva que calcWasiStage() en src/data/mock.ts (WASI_STAGE_THRESHOLDS).
      etapa = case
        when exp_total >= 22000 then 10
        when exp_total >= 18000 then 9
        when exp_total >= 14000 then 8
        when exp_total >= 12000 then 7
        when exp_total >= 8000 then 6
        when exp_total >= 6000 then 5
        when exp_total >= 4000 then 4
        when exp_total >= 2000 then 3
