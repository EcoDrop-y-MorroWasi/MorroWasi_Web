...
export default function CourseCard({ course, unlocked, completed = false, completedLessons, onOpen }: CourseCardProps) {
  ...
  return (
    <article className={`overflow-hidden rounded-2xl border-2 border-[#1c1c11] bg-[#fdfae7] text-[#1c1c11] ${HARD_SHADOW}`}>
      <img
        src={coverSrc}
        alt={`Portada del curso ${course.title}`}
        className="h-40 w-full border-b-2 border-[#1c1c11] object-cover"
      ...
      <div className="space-y-3 p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="shrink-0 rounded-full border-2 border-[#1c1c11] bg-[#FFB793] px-2 py-1 text-xs font-bold">+{course.xpReward} XP</span>
        </div>
        ...
        <div aria-label={`Progreso: ${completedLessons} de ${totalLessons} lecciones completadas`}>
          ...
          <div
            ...
            className="mt-1 h-2 w-full overflow-hidden rounded-full border-2 border-[#1c1c11] bg-white"
          >
        ...
        <button
          ...
          className={`min-h-12 w-full rounded-xl border-2 border-[#1c1c11] bg-[#99B4D8] px-4 py-3 text-sm font-bold transition-transform active:translate-x-[3px] active:translate-y-[3px] active:shadow-none disabled:cursor-not-allowed disabled:bg-stone-300 disabled:text-stone-600 disabled:shadow-none ${unlocked ? HARD_SHADOW : ""}`}
        >
