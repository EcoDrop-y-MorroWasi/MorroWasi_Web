...
export default function Album() {
  ...
  const reservoirPct = mockReservoir.currentLiters / mockReservoir.capacityLiters;
  ...
  return (
    <div className="mx-auto max-w-2xl pb-24">
      <Link
        to="/juegos"
        className="mb-4 inline-flex min-h-12 items-center gap-2 rounded-xl border-2 border-[#1c1c11] bg-white px-4 text-sm font-extrabold text-[#1c1c11] shadow-[2px_2px_0_#1c1c11] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none"
      >
        <span aria-hidden>←</span> Volver a Juegos
      </Link>
      <header className="mb-4 rounded-2xl border-2 border-[#1c1c11] bg-[#FFB793] p-5 shadow-[4px_4px_0_#1c1c11]">
        <p className="text-sm font-bold text-[#1c1c11]/70">Álbum de insignias</p>
        <h1 className="font-display text-2xl font-extrabold text-[#1c1c11]">🏅 Colección MorroWasi</h1>
        <p className="mt-1 text-sm font-bold text-[#1c1c11]">{unlockedCount}/6 insignias desbloqueadas</p>
      </header>
        ...
        {badges.map((b) => (
          <div
            ...
            className={`flex flex-col items-center gap-1 rounded-2xl border-4 border-white p-3 text-center transition-opacity ${
              b.unlocked ? "bg-white" : "bg-white/60 opacity-70 grayscale"
            }`}
          ...
          >
            ...
            <p className="text-xs font-black leading-tight text-[#1c1c11]">{b.title}</p>
            <p className="text-[10px] leading-snug text-[#1c1c11]/70">{b.description}</p>
            <span className="mt-1 rounded-full border-2 border-[#1c1c11] bg-[#99B4D8]/30 px-2 py-0.5 text-[10px] font-bold text-[#1c1c11]">
              {b.progressLabel}
            </span>
          </div>
        ))}
      ...
      <p className="mt-6 text-center text-[11px] text-[#1c1c11]/50">
