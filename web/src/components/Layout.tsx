...
// Bottom nav oficial 5 tabs (AGENTS.md §6 / UI_UX_Guide.md §2.1)
const TABS: NavTab[] = [
...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-bg-light font-body text-ink">
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-1 py-2">
          ...
          <div className="flex h-14 w-full items-center justify-between">
            ...
            <div className="hidden items-center gap-2 sm:flex">
              ...
              <button type="button" onClick={signOut} className="min-h-12 rounded-xl border-2 border-[#1c1c11] bg-secondary px-3 text-sm font-bold text-[#1c1c11] shadow-[2px_2px_0_#1c1c11] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none">Salir</button>
              <Link
                to="/perfil"
                aria-label={`Perfil de ${profile.name}`}
      ...
      >
        <div className="mx-auto flex h-20 w-full max-w-lg gap-1 px-1.5 sm:gap-1.5 sm:px-2">
          {TABS.map((tab) => (
            <NavLink
              key={tab.to}
              to={tab.to}
              ...
              className={({ isActive }) =>
                `my-2 flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 overflow-hidden rounded-xl border-2 border-[#1c1c11] px-0.5 text-[10px] font-semibold shadow-[2px_2px_0_#1c1c11] transition-colors active:translate-x-[2px] active:translate-y-[2px] active:shadow-none sm:gap-1 sm:text-xs ${
