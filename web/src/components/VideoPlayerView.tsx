...
export default function VideoPlayerView({ title, videoUrl, thumbnailUrl, onEnded }: VideoPlayerViewProps) {
  ...
  return (
    <section
      className="overflow-hidden rounded-2xl border-2 border-[#1c1c11] bg-[#1c1c11] shadow-[4px_4px_0_0_#1c1c11]"
    ...
    >
      {playing ? (
        ...
        >
          ...
          <span className="absolute inset-0 grid place-items-center">
            <span
              className="grid h-16 w-16 place-items-center rounded-full border-2 border-[#1c1c11] bg-[#FFB793] text-2xl shadow-[4px_4px_0_0_#1c1c11]"
              aria-hidden="true"
            >
              ▶
            </span>
      ...
      <div className="flex items-center justify-between gap-2 border-t-2 border-[#1c1c11] bg-[#fdfae7] px-3 py-2">
        <p className="truncate font-body text-xs font-bold text-[#1c1c11]">{title}</p>
        {youtube && (
          <span className="shrink-0 rounded-full border-2 border-[#1c1c11] bg-[#FFB793] px-2 py-0.5 text-[10px] font-bold text-[#1c1c11]">
            CC ES disponible
          </span>
        )}
      </div>
