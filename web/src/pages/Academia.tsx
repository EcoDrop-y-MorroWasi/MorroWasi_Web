...
export default function Academia({ hydroPoints = 0, wasiLevel = 1 }: AcademiaProps) {
  ...
  if (selected && stage === "detail")
    return (
      <main className="mx-auto max-w-3xl space-y-5 bg-[#fdfae7] px-4 py-6 text-[#1c1c11]">
        ...
        <section className={`rounded-2xl border-2 border-[#1c1c11] bg-[#99B4D8]/30 p-5 ${HARD_SHADOW}`} aria-label="Malla curricular del curso">
          <h2 className="font-display text-lg font-bold">📋 Malla curricular — qué aprenderás</h2>
          <ol className="mt-3 space-y-2">
            {selected.curriculum.map((item, i) => (
              <li key={item} className="flex items-start gap-3 rounded-xl border-2 border-[#1c1c11] bg-white p-3">
                <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full border-2 border-[#1c1c11] bg-[#FFB793] text-sm font-black">
                  {i + 1}
                </span>
        ...
        <button
          type="button"
          onClick={startCourse}
          className={`min-h-12 w-full rounded-xl border-2 border-[#1c1c11] bg-[#E26D5C] font-bold text-white ${HARD_SHADOW} active:translate-x-[2px] active:translate-y-[2px] active:shadow-none`}
        >
          Empezar curso
        </button>
      </main>
  ...
  if (selected && stage === "flashcards" && flashcardLesson)
    return (
      <main className="mx-auto max-w-3xl space-y-5 bg-[#fdfae7] px-4 py-6 text-[#1c1c11]">
        ...
        <section className={`rounded-2xl border-2 border-[#1c1c11] bg-white p-6 ${HARD_SHADOW}`} aria-label={`Flashcard: ${flashcardLesson.title}`}>
        ...
        <div className="flex gap-3">
          <button
            ...
            className={`min-h-12 flex-1 rounded-xl border-2 border-[#1c1c11] bg-white font-bold disabled:opacity-40 disabled:shadow-none ${HARD_SHADOW}`}
          >
            Anterior
          </button>
          <button
            ...
            className={`min-h-12 flex-1 rounded-xl border-2 border-[#1c1c11] bg-[#FFB793] font-bold ${HARD_SHADOW}`}
          >
  ...
  if (selected && stage === "quiz" && lesson)
    return (
      <main className="mx-auto max-w-3xl space-y-5 bg-[#fdfae7] px-4 py-6 text-[#1c1c11]">
        ...
        <section
          className={`rounded-2xl border-2 border-[#1c1c11] bg-[#99B4D8] p-5 ${HARD_SHADOW}`}
          ...
          <div className="mt-3 grid gap-2">
            {lesson.quiz.options.map((option, index) => (
              <button
                ...
                className={`min-h-12 rounded-xl border-2 border-[#1c1c11] px-4 py-3 text-left font-semibold ${answers[lesson.id] === index ? "bg-[#FFB793]" : "bg-white"}`}
              >
                {option}
              </button>
            ))}
        ...
        <div className="flex gap-3">
          ...
          {lessonIndex < selected.lessons.length - 1 ? (
            ...
            <button
              type="button"
              onClick={closeCourse}
              className={`min-h-12 flex-1 rounded-xl border-2 border-[#1c1c11] bg-[#E26D5C] font-bold text-white ${HARD_SHADOW}`}
            >
              Finalizar
            </button>
          )}
  ...
  return (
    <main className="mx-auto max-w-6xl bg-[#fdfae7] px-4 py-6 text-[#1c1c11]">
      <header className={`mb-6 rounded-2xl border-2 border-[#1c1c11] bg-[#99B4D8] p-5 ${HARD_SHADOW}`}>
