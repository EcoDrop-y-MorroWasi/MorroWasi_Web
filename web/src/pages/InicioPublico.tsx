...
const SECTIONS = [
  ...
  { icon: '📍', title: 'Dónde nació', tone: 'bg-white', text: 'MorroWasi nace en Morropón, Piura, como parte del Desafío Amauta 2026: una respuesta local, alegre y práctica para una necesidad que compartimos.' },
  ...
  return (
    <main className="min-h-screen bg-bg-light px-4 py-6 text-ink sm:py-10">
      <div className="mx-auto flex max-w-5xl flex-col gap-8">
        ...
        <section className="keyline-border rounded-2xl bg-white p-5 shadow-[4px_4px_0_var(--color-ink)]" aria-labelledby="galeria-title">
          <div className="flex flex-wrap items-end justify-between gap-2"><div><h2 id="galeria-title" className="font-display text-2xl font-extrabold">Galería de aprendizaje</h2><p className="mt-1 font-semibold text-ink/80">Fotos reales de referencia — de dominio público o licencia Creative Commons.</p></div><span className="rounded-full border-2 border-[#1c1c11] bg-secondary px-3 py-1 text-sm font-bold">6 recursos</span></div>
...
function AccessLink({ label, extra = 'bg-primary shadow-[2px_2px_0_var(--color-ink)]' }: { label: string; extra?: string }) {
  return <Link to="/login" className={`inline-flex min-h-12 items-center justify-center rounded-xl border-2 border-[#1c1c11] px-5 font-display font-bold active:translate-x-[2px] active:translate-y-[2px] active:shadow-none ${extra}`}>{label}</Link>
}
