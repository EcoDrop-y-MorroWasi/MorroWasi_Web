...
export default function Noticias() {
  // Fuente de verdad morrowasi-preview.html: totalSavedLiters=2340, goalLiters=5000
  const META = 5000;
  ...
  const add = (l: number) => {
    setTotal((t) => {
      ...
      if (t < META && next >= META) fireConfetti();
      return next;
    });
  ...
  return (
    <div className="relative mx-auto max-w-[800px] p-4 pb-24 space-y-4">
      ...
      <h2 className="text-xl font-extrabold text-[#1c1c11]">📰 Noticias y Ahorro</h2>
      ...
      >
        ...
        <div
          className="mt-4 h-5 rounded-full bg-white/60 border-2 border-[#1c1c11] overflow-hidden"
          role="progressbar"
          aria-valuenow={pct}
          aria-valuemin={0}
          aria-valuemax={100}
        ...
        <motion.button
          ...
          onClick={reset}
          className="mt-3 min-h-[48px] rounded-lg bg-white border-2 border-[#1c1c11] px-4 font-bold text-xs shadow-[2px_2px_0_#1c1c11] active:shadow-none"
        >
          Reiniciar (demo)
        </motion.button>
      </section>
      ...
      <section className="rounded-xl bg-white keyline shadow-[4px_4px_0_#1c1c11] p-4">
        <h3 className="font-extrabold text-[#1c1c11] mb-3">⚡ Registro rápido</h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {QUICK_SAVE.map((q) => (
            <motion.button
              ...
              className="flex min-h-[96px] flex-col items-center justify-center gap-1 rounded-xl bg-white keyline shadow-[2px_2px_0_#1c1c11] hover:bg-[#FFB793] active:shadow-none transition-colors"
              aria-label={`Agregar ${q.liters} litros por ${q.label}`}
            >
              <span className="text-2xl" aria-hidden>
                {q.emoji}
              </span>
              <span className="text-sm font-extrabold text-[#1c1c11]">{q.label}</span>
      ...
      <section className="rounded-xl bg-white keyline shadow-[4px_4px_0_#1c1c11] p-4">
        <h3 className="font-extrabold text-[#1c1c11] mb-2">🧮 Calculadora de costo por desperdicio (S/)</h3>
        <label className="text-sm font-bold text-[#1c1c11]" htmlFor="wasteRange">
        ...
        <div className="flex justify-between items-center mt-1">
          <span className="text-lg font-black text-[#1c1c11]">{wasteLiters} L</span>
          <span className="text-sm font-extrabold text-[#E26D5C]">S/ {costo.toFixed(2)} al mes</span>
        </div>
        <p className="text-xs text-[#1c1c11]/60">Tarifa referencial EPS Grau (operador de agua en Piura): S/ {TARIFA_SOLES_POR_LITRO.toFixed(2)} por litro.</p>
        <div className="mt-3 grid grid-cols-3 gap-2 text-center">
          <div className="rounded-lg bg-[#fdfae7] border-2 border-[#1c1c11] p-2">
            <div className="text-xs font-bold">Al año</div>
            <div className="text-sm font-black text-[#E26D5C]">S/ {(costo * 12).toFixed(2)}</div>
          </div>
          <div className="rounded-lg bg-[#99B4D8] border-2 border-[#1c1c11] p-2">
          ...
          <div className="rounded-lg bg-[#FFB793] border-2 border-[#1c1c11] p-2">
      ...
      <section className="rounded-xl bg-white keyline shadow-[4px_4px_0_#1c1c11] p-4" aria-label="Boletín de noticias sobre el agua en Piura y Morropón">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <h3 className="font-extrabold text-[#1c1c11]">📰 Agua en Piura y Morropón</h3>
          {feedStatus === "ok" && (
            <span className="rounded-full border-2 border-[#1c1c11] bg-[#99B4D8]/40 px-2 py-0.5 text-[10px] font-black">
              🔴 En vivo
            </span>
          )}
        </div>
        ...
        {feedStatus === "cargando" && (
          <ul className="space-y-3" aria-label="Cargando noticias">
            {[0, 1, 2].map((i) => (
              <li key={i} className="h-20 animate-pulse rounded-xl border-2 border-[#1c1c11]/20 bg-[#fdfae7]" />
            ))}
          </ul>
        )}
          ...
          <>
            <ul className="space-y-3" role="list">
              {noticiasFeed.map((item) => (
                <li key={item.id} className="rounded-xl border-2 border-[#1c1c11] bg-[#fdfae7] p-3 shadow-[2px_2px_0_#1c1c11]">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-xs font-bold text-[#1c1c11]/60">{item.date} · {item.source}</span>
                    <span className="rounded-full border-2 border-[#1c1c11] bg-[#FFB793] px-2 py-0.5 text-[10px] font-black">{item.tag}</span>
                  </div>
                  <a href={item.url} target="_blank" rel="noreferrer" className="mt-1 block font-display text-base font-bold text-[#1c1c11] hover:underline">
                    {item.title}
                  </a>
                  <p className="mt-1 text-sm text-[#1c1c11]/80 leading-snug">{item.snippet}</p>
                </li>
              ))}
            </ul>
          </>
        ...
        {feedStatus === "error" && (
          <>
            <ul className="space-y-3" role="list">
              {NEWS.map((item) => (
                <li key={item.title} className="rounded-xl border-2 border-[#1c1c11] bg-[#fdfae7] p-3 shadow-[2px_2px_0_#1c1c11]">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-xs font-bold text-[#1c1c11]/60">{item.date} · {item.source}</span>
                    <span className="rounded-full border-2 border-[#1c1c11] bg-[#FFB793] px-2 py-0.5 text-[10px] font-black">{item.tag}</span>
                  </div>
            ...
            <p className="mt-3 text-[11px] text-[#1c1c11]/50">Sin conexión al feed en vivo — mostrando boletín verificado de MorroWasi.</p>
          </>
        )}
      </section>
    </div>
